 ...
 * @category Datasource
 */
export declare type ListenEntityProps<M> = FetchEntityProps<M> & {
 ...
 * @category Datasource
 */
export declare type ListenCollectionProps<M> = FetchCollectionProps<M> & {
...
export interface DataSource {
     ...
     */
    fetchCollection<M>({ path, schema, filter, limit, startAfter, orderBy, order, searchString }: FetchCollectionProps<M>): Promise<Entity<M>[]>;
    /**
     ...
     * @see useCollectionFetch if you need this functionality implemented as a hook
     */
    listenCollection?<M>({ path, schema, filter, limit, startAfter, searchString, orderBy, order, onUpdate, onError }: ListenCollectionProps<M>): () => void;
    /**
     ...
     * @param schema
     */
    fetchEntity<M>({ path, entityId, schema }: FetchEntityProps<M>): Promise<Entity<M>>;
    /**
     ...
     * @return Function to cancel subscription
     */
    listenEntity?<M>({ path, entityId, schema, onUpdate, onError }: ListenEntityProps<M>): () => void;
    /**
     ...
     * @param status
     */
    saveEntity<M>({ path, entityId, values, schema, status }: SaveEntityProps<M>): Promise<Entity<M>>;
    /**
     * Delete an entity
     * @param entity
